...
def generate_policy(cfg, bs, device,type):
    ...
    if (type == "search"):
        ...
        search_policy = torch.zeros([bs, search_feat_size * search_feat_size, 4], device=device)
        search_policy[:, :, 2] = 1
        return search_policy
    if (type == "template"):
        template_size = cfg.DATA.TEMPLATE.SIZE
        template_feat_size = template_size // stride
        template_policy = torch.zeros([bs, template_feat_size * template_feat_size, 4], device=device)
        template_policy[:, :, 0] = 1
        return template_policy
    if (type == "dynamic"):
        dynamic_size = cfg.DATA.TEMPLATE.SIZE
        dynamic_feat_size = dynamic_size // stride
        dynamic_policy = torch.zeros([bs, dynamic_feat_size * dynamic_feat_size, 4], device=device)
        dynamic_policy[:, :, 1] = 1
        return dynamic_policy


def generate_mask(policy):
    ...
    N = search_policy.shape[1] + template_policy.shape[1] + dynamic_policy.shape[1]
    B = search_policy.shape[0]
    Group_T = final_policy[:, :, 0].reshape(B, 1, N, 1) @ final_policy[:, :, 0].reshape(B, 1, 1, N) + \
              final_policy[:, :, 0].reshape(B, 1, N, 1) @ final_policy[:, :, 1].reshape(B, 1, 1, N) + \
              final_policy[:, :, 0].reshape(B, 1, N, 1) @ final_policy[:, :, 3].reshape(B, 1, 1, N)
    Group_D = final_policy[:, :, 1].reshape(B, 1, N, 1) @ final_policy[:, :, 0].reshape(B, 1, 1, N) + \
              final_policy[:, :, 1].reshape(B, 1, N, 1) @ final_policy[:, :, 1].reshape(B, 1, 1, N) + \
              final_policy[:, :, 1].reshape(B, 1, N, 1) @ final_policy[:, :, 3].reshape(B, 1, 1, N)

    Group_S = final_policy[:, :, 2].reshape(B, 1, N, 1) @ final_policy[:, :, 0].reshape(B, 1, 1, N) + \
              final_policy[:, :, 2].reshape(B, 1, N, 1) @ final_policy[:, :, 1].reshape(B, 1, 1, N) + \
              final_policy[:, :, 2].reshape(B, 1, N, 1) @ final_policy[:, :, 2].reshape(B, 1, 1, N) + \
              final_policy[:, :, 2].reshape(B, 1, N, 1) @ final_policy[:, :, 3].reshape(B, 1, 1, N) + \
              final_policy[:, :, 3].reshape(B, 1, N, 1) @ final_policy[:, :, 0].reshape(B, 1, 1, N) + \
              final_policy[:, :, 3].reshape(B, 1, N, 1) @ final_policy[:, :, 1].reshape(B, 1, 1, N) + \
              final_policy[:, :, 3].reshape(B, 1, N, 1) @ final_policy[:, :, 2].reshape(B, 1, 1, N) + \
              final_policy[:, :, 3].reshape(B, 1, N, 1) @ final_policy[:, :, 3].reshape(B, 1, 1, N)
    mask = Group_T + Group_D + Group_S
    return mask
...
def generate_bbox_mask(bbox_mask, bbox):
    ...
    for i in range(b):
        ...
        if int(bbox_i[3]) > 256:
            bbox_i[3] = 256
        bbox_mask[i, int(bbox_i[1]):int(bbox_i[1] + bbox_i[3] - 1),
        int(bbox_i[0]):int(bbox_i[0] + bbox_i[2] - 1)] = 1
    return bbox_mask
...
def generate_mask_cond(cfg, bs, device, gt_bbox):
    ...
    if cfg.MODEL.BACKBONE.CE_TEMPLATE_RANGE == 'ALL':
        ...
        box_mask_z = box_mask_z.flatten(1)
